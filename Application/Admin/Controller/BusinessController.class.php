...
class BusinessController extends BaseController {
    ...
    public function modifyuser(){
		...
		if($doinfo=="modifyuser"){
			...
			if($sdata["typeid"]==""){
				echo "<script>alert(\"请选择用户组\");location.href='".U('Business/modifyuser','id='.$id)."';</script>";
				exit();
			}elseif($sdata["username"]==""){
				echo "<script>alert(\"用户名不能为空\");location.href='".U('Business/modifyuser','id='.$id)."';</script>";
				exit();
			}
			...
			if(empty($id)){
				...
				$company_id = $sysuser->add($sdata);
				//创建店铺
				$entity = array('is_self'=>0,'type'=>$type,'status'=>1,'badmin_id'=>$company_id,'title'=>$sdata["truename"],'head'=>'','head'=>'/Public/Admin/img/company_default.png','content'=>'','collect'=>0,'createtime'=>date('Y-m-d H:i:s'));
				$business_id = D('business')->add($entity);
				//更新商家账号的店铺ID
				$sysuser->where("id=$company_id")->save(array('business_id'=>$business_id));
			}else{
				$sysuser->where("id=$id")->save($sdata);
			}
			echo "<script>alert(\"操作成功\");location.href='".U('Business/listuser')."';</script>";
			exit();
		}
		$this->assign($data);
    	$this->show();
    ...
    public function deluserclass(){
		...
		if($sysuserclassinfo["title"]!="系统管理员"){
			...
			if(count($sysuserinfo)>0){
				...
				redirect(U("Business/listuserclass"));
			}
		}else{
			...
			//资料修改
				...
				if($fdata["bid"]=="0"){//上一级为一级
					...
					if($cdata["bid"]==$fdata["bid"]){
						...
						redirect(U("Business/listsysitem"));
						exit();
					}else{
						//改变了上一级
						//修改后的位置在最后
	...
	public function sysitemsortad(){
        ...
        if(count($id)>0){
            ...
            $this->redirect("Business/listsysitem");
            exit();
        }else{
            $this->assign("jumpUrl", U("Business/listsysitem"));
            $this->error("没有进行任何操作");
            exit();
        }
	}
    ...
    public function modifypassword(){
    	...
    	if($doinfo=="modifypassword"){
			...
			if($res){
				echo "<script>alert('修改成功!');location.href='".U('Business/modifypassword')."';</script>";
				exit();
			}else{
				echo "<script>alert('没有任何修改!');location.href='".U('Business/modifypassword')."';</script>";
				exit();
			}
	...
	public function sysconfig(){
		...
		if($doinfo=="sysconfig"){
			...
			echo "<script>alert('修改成功!');location.href='".U('Business/sysconfig')."';</script>";
			exit();
		}
