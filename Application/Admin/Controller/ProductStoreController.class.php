...
class ProductStoreController extends BaseController {
    ...
    public function listad(){
        ...
        $data = $this->pager(array("mo"=>$model, "count"=>$count), "10", "updatetime desc,createdate desc", $map);
        $this->assign($data);
    ...
    public function modifyad(){
        ...
        if($doinfo == "modify"){
            ...
            $this->redirect("ProductStore/listad", $this->getMap());
        }
        if($id){
    ...
    public function sortad(){
        ...
        if(count($id)>0){
            ...
            $this->assign("jumpUrl", U("ProductStore/listad", $this->getMap()));
            $this->error("没有进行任何操作");
            exit();
        }
    }
