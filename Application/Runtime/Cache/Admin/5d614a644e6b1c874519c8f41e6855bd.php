...
<body class="gray-bg">
    <form id="form1" name="form1" method="post" action="<?php echo U('category/sortad','p='.$map['p'].'&type='.$map['type'].'&depth='.$depth);?>">
        <div class="wrapper wrapper-content animated fadeInRight">
            <div class="row">
                <div class="col-sm-12">
                    <div class="ibox float-e-margins">
                        <div class="ibox-content">
                            <button class="btn btn-primary" type="button" onclick="location.href='<?php echo U('Category/modifyad', 'p='.$map['p'].'&depth='.$depth);?>'">＋增加</button>
                            ...
                            <table class="table table-striped table-hover">
                                <thead>
                                    <tr>
                                        ...
                                        <?php if($depth==1): ?><th width="200" class="TC">所属一级分类</th><?php endif; ?>
                                        <?php if($depth==2): ?><th width="200" class="TC">所属二级分类</th><?php endif; ?>
                                ...
                                <tbody>
                                    <?php if(is_array($data)): $n = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($n % 2 );++$n;?><tr>
                                            ...
                                            <?php if($depth==1): ?><td align="center">
                                                    <?php echo ($vo["column_name"]); ?>
                                                </td><?php endif; ?>
                                            <?php if($depth==2): ?><td align="center"><?php if($vo.category_pid!=null): echo ($vo["category_p_name"]); else: ?>无<?php endif; ?></td><?php endif; ?>
                                            <td align="center"><?php echo (date("Y-m-d",strtotime($vo["createdate"]))); ?></td>
                                            <td class="text-navy" align="center">
                                                <a href="<?php echo U('Category/modifyad','id='.$vo['id'].'&p='.$map['p'].'&depth='.$depth);?>">修改</a>
                                                <a href="javascript:;" data-url="<?php echo U('Category/delad','id='.$vo['id'].'&p='.$map['p'].'&depth='.$depth);?>"
                                                    class="checkDel">删除</a>
                                            </td>
