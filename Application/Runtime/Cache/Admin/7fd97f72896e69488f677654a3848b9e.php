                ...
                <div class="ibox float-e-margins">
                    <div class="ibox-content">
                        <form method="post" class="form-horizontal"
                            action="<?php echo U('Sysuser/modifyuserclass','doinfo=modify&id='.$datainfo['id']);?>">
                            <div class="form-group">
                                <label class="col-sm-2 control-label">用户组名称：</label>

                                <div class="col-sm-10">
                                    <input type="text" class="form-control" name="title" value="<?php echo ($datainfo["title"]); ?>"
                                        required="">
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-10 checkbox">
                                    <?php if(is_array($sysitem)): $i = 0; $__LIST__ = $sysitem;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="J-container">
                                            <label class="W1 J-menu" id="<?php echo ($vo["id"]); ?>">
                                                <input type="checkbox" class="i-checks" name="power[]" value="<?php echo ($vo["id"]); ?>"
                                                    <?php echo (getSelect(check,$vo["id"],$datainfo["power"])); ?>>
                                                <i></i><?php echo ($vo["title"]); ?>
                                            </label>
                                            <div class="J-child-<?php echo ($vo["id"]); ?>" style="margin-bottom:20px;margin-left:25px;">
                                                <?php if(is_array($vo["child"])): $i = 0; $__LIST__ = $vo["child"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ivo): $mod = ($i % 2 );++$i;?><label class="W1">
                                                        <input type="checkbox" class="i-checks" name="power[]"
                                                            value="<?php echo ($ivo["id"]); ?>"
                                                            <?php echo (getSelect(check,$ivo["id"],$datainfo["power"])); ?>>
                                                        <i></i><?php echo ($ivo["title"]); ?>
                            ...
                            <div class="form-group">
                                <div class="col-sm-4 col-sm-offset-2">
                                    ...
                                    <button class="btn btn-white" type="button"
                                        onclick="location.href='<?php echo U('Sysuser/listuserclass');?>'">返回</button>
                                </div>
                            </div>
                        </form>
                    </div>
