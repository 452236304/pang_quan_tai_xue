            ...
            <div class="ibox float-e-margins">
                <div class="ibox-content">
                    <form method="post" class="form-horizontal" enctype="multipart/form-data"
                          action="<?php echo U('CaseContent/modify','doinfo=modify&id='.$info['id'].'&p='.$map['p']);?>">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-10">
                                <input name="img_url" id="img_url" type="text" class="form-control"
                                       value="<?php echo ($info["img_url"]); ?>"  required="" /><br>
                                <div>
                                    <a onclick="BrowseServer('Images:/', 'img_url');" href="javascript:void(0);"
                                       class="btn btn-white" data-options="iconCls:'icon-redo'"
                                       style="width: 60px">选择</a>
                                    <a id="img_url_Img" title="点击预览大图,点击大图关闭预览" href="<?php echo ($info["img_url"]); ?>"
                                       target="_blank">
                                        <?php if($info["img_url"] != ''): ?><img src="<?php echo ($info["img_url"]); ?>"
                                                 style="max-height:100px;max-width: 100%" /><?php endif; ?>
                                    </a>
                                    <div class="red" style="margin-top:5px;">尺寸大小为：200*100</div>
                                </div>
                            </div>
                        </div>
                        <div class="hr-line-dashed"></div>
                        <div class="form-group">
                            <label class="col-sm-2 control-label">概述：<span style="color:red;margin:5px;">*</span></label>
                            <div class="col-sm-10">
							        <textarea class="form-control" name="content"
                                              style="resize:none;height:100px;"><?php echo ($info["content"]); ?></textarea>
                            </div>
                        </div>
                        <div class="hr-line-dashed"></div>
                        <div class="form-group">
                            ...
                            <div class="col-sm-10" style="position: relative;">
                                    <textarea cols="80" id="content" name="content_detail" style="height: 400px;"
                                              rows="10" required="请填写内容" ><?php echo ($info["content_detail"]); ?></textarea>
                            </div>
                        </div>
                        <div class="hr-line-dashed"></div>
                        <div class="form-group">
                            ...
                            <div class="col-sm-10">
                                <select data-placeholder="选择分类" name="cate_id"
                                        class="form-control chosen-select" required>
                                    <option value="0">选择分类</option>
                                    <?php if(is_array($cate)): foreach($cate as $k=>$v): ?><option value="<?php echo ($v["id"]); ?>" <?php echo (getSelect(select,$v["id"],$info["cate_id"])); ?>>
                                        <?php echo ($v["title"]); ?></option><?php endforeach; endif; ?>
                                </select>
                            </div>
                        </div>
                        <div class="hr-line-dashed"></div>
                        <div class="form-group">
                            ...
                            <div class="col-sm-10">
                                <input type="text" class="form-control" name="orderby"
                                       value="<?php echo ((isset($info["orderby"]) && ($info["orderby"] !== ""))?($info["orderby"]):999); ?>"
                                       required="">
                            </div>
                        </div>
                        <div class="hr-line-dashed"></div>
                        <div class="form-group myform">
                            <label class="col-sm-2 control-label">是否显示<span style="color:red;margin:5px;">*</span>：</label>
                            <div class="col-sm-10 radio i-checks">
                                <label><input type="radio" name="status" value="0"
                                              <?php echo (getSelect(radio,$info["status"],0)); ?>><i></i>隐藏</label>
                                <label><input type="radio" name="status" value="1"
                                              <?php echo (getSelect(radio,$info["status"],1)); ?>><i></i>显示</label>
                            </div>
                        </div>
                        <div class="hr-line-dashed"></div>
                        <div class="form-group">
                            <div class="col-sm-4 col-sm-offset-2">
                                ...
                                <button class="btn btn-white" type="button"
                                        onclick="location.href='<?php echo U('CaseContent/index','p='.$map['p']);?>'">返回</button>
                            </div>
                        </div>
                    </form>
                </div>
