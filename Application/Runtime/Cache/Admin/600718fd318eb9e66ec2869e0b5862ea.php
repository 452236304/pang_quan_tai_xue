...
<body class="gray-bg">
    <form id="form1" name="form1" method="get"
        action="<?php echo U('ProductOrder/listad', 'type='.$map['type'].'&status='.$map['status']);?>">
        <div class="wrapper wrapper-content animated fadeInRight">
            <div class="row">
				<?php if($_GET['status']!='sh'){ ?>
				<div class="col-sm-12">
                    <div class="ibox float-e-margins row">
						<div class="col-sm-4 header_title <?php echo $_GET['type']=='0'?'header_active':''; ?>" onclick="javascript:location.href='<?php echo U('product_order/listad').'?type=0';?>'">商品订单</div>
						<div class="col-sm-4 header_title <?php echo $_GET['type']=='1'?'header_active':''; ?>" onclick="javascript:location.href='<?php echo U('product_order/listad').'?type=1';?>'">定制订单</div>
						<div class="col-sm-4 header_title <?php echo $_GET['type']=='2'?'header_active':''; ?>" onclick="javascript:location.href='<?php echo U('product_order/listad').'?type=2';?>'">改造订单</div>
					</div>
				</div>
				<?php }?>
                <div class="col-sm-12">
                    <div class="ibox float-e-margins row">
                        <div class="col-sm-2">
							<select class="form-control" name='status'>
								...
								<option value="1" <?php echo (getSelect(select,1,$where["status"])); ?> >待付款</option>
								<option value="3" <?php echo (getSelect(select,3,$where["status"])); ?> >已付款</option>
								<option value="2" <?php echo (getSelect(select,2,$where["status"])); ?> >已取消</option>
								<option value="4" <?php echo (getSelect(select,4,$where["status"])); ?> >已完成</option>
								<option value="5" <?php echo (getSelect(select,5,$where["status"])); ?> >申请售后</option>
								<option value="6" <?php echo (getSelect(select,6,$where["status"])); ?> >售后已完成</option>
							</select>
						</div> 
						<div class="col-sm-2">
						...
						<div class="col-sm-2">
							<select class="form-control" name="is_activity">
								...
								<option value='0' <?php echo (getSelect(select,'0',$where["is_activity"])); ?> >普通订单</option>
								<option value="1" <?php echo (getSelect(select,1,$where["is_activity"])); ?> >活动订单</option>
							</select>
						</div> 
                            ...
                            <table class="table table-striped table-hover">
                                <thead>
                                    <tr>
                                        ...
                                        <?php if($map["type"] == 0): ?><th width="150" class="TC">收货人信息</th>
                                            <th width="150" class="TC">快递信息</th><?php endif; ?>
                                ...
                                <tbody>
                                    <?php if(is_array($data)): $n = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($n % 2 );++$n;?><tr>
											...
											<td align="center"><?php echo ($vo['is_activity'] == 1?'活动订单':'普通订单'); ?></td>
                                            ...
                                            <td align="center">
                                                <?php if(($vo["status"] == 1) and ($vo["pay_status"] == 0)): ?>待付款
                                                    <?php elseif(($vo["status"] == 1) and ($vo["pay_status"] == 3) and ($vo["examine"] == 1)): ?>
                                                    已付款
                                                    ...
                                                    售后已完成
													<?php elseif($vo["examine"] == 0): ?>
													待审核
													<?php elseif($vo["examine"] == 2): ?>
													审核不通过
                                                    <?php else: ?>
                                                    未知状态<?php endif; ?>
                                            </td>
                                            <?php if($map["type"] == 0): ?><td align="center">
                                                    <?php echo ($vo["consignee"]); ?>，<?php echo ($vo["mobile"]); ?><br />
                                                    <?php echo ($vo["province"]); echo ($vo["city"]); echo ($vo["region"]); ?><br />
                                                    <?php echo ($vo["address"]); ?>
                                                </td>
                                                <td align="center">
                                                    <?php if($vo["shipping_status"] == 0): ?>待发货
                                                        <?php elseif($vo["shipping_status"] == 1): ?>
                                                        已发货<br /><?php echo ($vo["shipping_name"]); ?>，<?php echo ($vo["shipping_number"]); ?><br /><?php echo (date("Y-m-d
                                                        H:i",strtotime($vo["shipping_send_date"]))); ?>
                                                        <?php elseif($vo["shipping_status"] == 2): ?>
                                                        已收货<br /><?php echo ($vo["shipping_name"]); ?>，<?php echo ($vo["shipping_number"]); ?><br /><?php echo (date("Y-m-d
                                                        H:i",strtotime($vo["shipping_receive_date"]))); ?>
                                                        <?php else: ?>
                                                        未知状态<?php endif; ?>
                                                </td><?php endif; ?>
                                            <td align="center">
											...
											<td align="center"><?php echo ($vo["coupon_money"]); ?>元</td>
											<td align="center"><?php echo ($vo["amount"]); ?>元</td>
											<td align="center"><?php echo ($vo["total_amount"]); ?>元</td>
                                            ...
                                            <td class="text-navy" align="center">
                                                <?php if($map["type"] == 1): ?><a href="<?php echo U('ProductOrderAttach/modifyad','orderid='.$vo['id']);?>"
                                                        class="J-OpenTab" data-index="order_attach_<?php echo ($vo["id"]); ?>"
                                                        data-title="订单<?php echo ($vo["sn"]); ?>-定制信息">定制信息</a><br>
                                                    <?php elseif($map["type"] == 2): ?>
                                                    <a href="<?php echo U('ProductOrderAttach/modifyad','orderid='.$vo['id']);?>"
                                                        class="J-OpenTab" data-index="order_attach_<?php echo ($vo["id"]); ?>"
                                                        data-title="订单<?php echo ($vo["sn"]); ?>-改造信息">改造信息</a><br>
                                                    <?php elseif($map["status"] == 'sh'): ?>
                                                    <a href="<?php echo U('ProductOrderRefund/modifyad','orderid='.$vo['id']);?>"
                                                        class="J-OpenTab" data-index="order_refund_<?php echo ($vo["id"]); ?>"
                                                        data-title="商品订单<?php echo ($vo["sn"]); ?>-售后原因">售后原因</a><br>
                                                    <a href="<?php echo U('ProductOrderProduct/listad','orderid='.$vo['id']);?>"
                                                        class="J-OpenTab" data-index="order_product_<?php echo ($vo["id"]); ?>"
                                                        data-title="订单<?php echo ($vo["sn"]); ?>-订单商品">订单商品</a><br>
                                                    <?php else: ?>
                                                    <a href="<?php echo U('ProductOrderProduct/listad','orderid='.$vo['id']);?>"
                                                        class="J-OpenTab" data-index="order_product_<?php echo ($vo["id"]); ?>"
                                                        data-title="订单<?php echo ($vo["sn"]); ?>-订单商品">订单商品</a><br>
                                                    <?php if(($vo["shipping_status"] == 0) and ($vo["status"] == 1) and ($vo["pay_status"] == 3) and ($vo["examine"] == 1)): ?><a href="<?php echo U('ProductOrder/shipping','id='.$vo['id'].'&p='.$map['p'].'&type='.$map['type']);?>">订单发货</a><br><?php endif; ?>
													 <?php if( ($vo["status"] == 1) and ($vo["pay_status"] == 3) and ($vo["examine"] == 0)): ?><a href="<?php echo U('ProductOrder/examine','id='.$vo['id'].'&p='.$map['p'].'&type='.$map['type'].'&keyword='.$map['keyword'].'&examine=1');?>">审核通过</a><br>
														<a href="<?php echo U('ProductOrder/examine','id='.$vo['id'].'&p='.$map['p'].'&type='.$map['type'].'&keyword='.$map['keyword'].'&examine=2');?>">审核拒绝</a><br><?php endif; endif; ?>
                                                <?php if($vo["is_comment"] > 0 and $map["type"] == 0): ?><a href="<?php echo U('ProductOrderProduct/listad','orderid='.$vo['id']);?>"
                                                        class="J-OpenTab" data-index="product_order_product_<?php echo ($vo["id"]); ?>"
                                                        data-title="订单<?php echo ($vo["sn"]); ?>-订单商品">查看评论</a><br><?php endif; ?>
                                                <?php if($vo["is_comment"] > 0 and $map["type"] > 0): ?><a href="<?php echo U('ProductOrder/comment','orderid='.$vo['id']);?>"
                                                        class="J-OpenTab" data-index="order_product_<?php echo ($vo["id"]); ?>"
                                                        data-title="订单<?php echo ($vo["sn"]); ?>-查看评论">查看评论</a><br><?php endif; ?>
												<?php if(($vo["status"] == 1) and ($vo["pay_status"] == 0)): ?><a href="<?php echo U('ProductOrder/change_price','id='.$vo['id'].'&p='.$map['p']);?>">修改订单价格</a><br><?php endif; ?>
