...
<body class="gray-bg">
    <div class="wrapper wrapper-content animated fadeInRight">
        <form id="form1" name="form1" method="post" action="<?php echo U('Sysuser/sysitemsortad');?>">
            <div class="row">
                <div class="col-sm-12">
                    <div class="ibox float-e-margins">
                        <div class="ibox-content">
                            <button class="btn btn-primary" type="button" onclick="location.href='<?php echo U('Sysuser/modifysysitem');?>'">＋增加</button>
                    ...
                    <div class="ibox float-e-margins">
                        <div class="ibox-content">
                            <table class="table table-striped table-hover">
                                ...
                                <tbody>
                                    <?php if(is_array($sysitem)): $n = 0; $__LIST__ = $sysitem;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($n % 2 );++$n;?><tr class="class<?php echo ($vo["parentid"]); ?>" onclick="exSubinfo('.class<?php echo ($vo["id"]); ?>')">
                                            <td><?php echo ($n); ?><input type="hidden" name="id[]" value="<?php echo ($vo["id"]); ?>" /></td>
                                            <td>
                                                <?php if($vo["childnum"] != 0): ?><i class="fa fa-plus-square-o"></i>
                                                    <?php else: ?>
                                                    <i class="fa fa-minus-square-o"></i><?php endif; ?>
                                                <?php echo ($vo["title"]); ?>
                                            </td>
                                            <td>
                                                <input type="text" style="text-align:center; width:50px;" class="form-control"
                                                    name="orderby[]" value="<?php echo ($vo["orderby"]); ?>" />
                                            </td>
                                            <td><?php echo ($vo["url"]); ?></td>
                                            <td class="text-navy">
                                                <a href="<?php echo U('Sysuser/modifysysitem','id='.$vo['id']);?>">修改</a>
                                                <a href="javascript:;" data-url="<?php echo U('Sysuser/delsysitem','id='.$vo['id']);?>"
                                                    class="checkDel">删除</a>
                                                <!-- <?php if(getpower($channel,'modify') == 1): ?><a href="<?php echo U('Sysuser/modifysysitem','id='.$vo['id']);?>" >修改</a><?php endif; ?>
                                            <?php if(getpower($channel,'del') == 1): ?><a href="javascript:;" data-url="<?php echo U('Sysuser/delsysitem','id='.$vo['id']);?>" class="checkDel">删除</a><?php endif; ?> -->
                                            </td>
                                        </tr>
                                        <?php if(is_array($vo['child'])): $in = 0; $__LIST__ = $vo['child'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ivo): $mod = ($in % 2 );++$in;?><tr class="class<?php echo ($ivo["parentid"]); ?>">
                                                <td style="padding-left:25px;"><?php echo ($in); ?><input type="hidden" name="id[]"
                                                        value="<?php echo ($ivo["id"]); ?>" /></td>
                                                <td style="padding-left:25px;">
                                                    <?php echo ($ivo["title"]); ?>
                                                </td>
                                                <td>
                                                    <input type="text" style="text-align:center; width:50px;" class="form-control"
                                                        name="orderby[]" value="<?php echo ($ivo["orderby"]); ?>" />
                                                </td>
                                                <td><?php echo ($vo["url"]); ?></td>
                                                <td class="text-navy">
                                                    <a href="<?php echo U('Sysuser/modifysysitem','id='.$ivo['id']);?>">修改</a>
                                                    <a href="javascript:;" data-url="<?php echo U('Sysuser/delsysitem','id='.$ivo['id']);?>"
                                                        class="checkDel">删除</a>
                                                    <!-- <?php if(getpower($channel,'modify') == 1): ?><a href="<?php echo U('Sysuser/modifysysitem','id='.$vo['id']);?>" >修改</a><?php endif; ?>
                                                <?php if(getpower($channel,'del') == 1): ?><a href="javascript:;" data-url="<?php echo U('Sysuser/delsysitem','id='.$vo['id']);?>" class="checkDel">删除</a><?php endif; ?> -->
                                                </td>
