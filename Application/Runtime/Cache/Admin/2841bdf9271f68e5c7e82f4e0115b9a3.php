            ...
            <div class="col-sm-12">
                <div class="ibox float-e-margins">
                    <div class="ibox-content">
                        <form method="post" class="form-horizontal" action="<?php echo U('Sysuser/modifysysitem','doinfo=modifysysitem&id='.$sysitem['id']);?>">
                            <div class="form-group">
                                <label class="col-sm-2 control-label">上一级：</label>

                                <div class="col-sm-10">
                                    <select data-placeholder="选择上一级..." class="chosen-select form-control" style="width:350px;"
                                        tabindex="2" name="bid">
                                        <option value="0">作为一级项目</option>
                                    <?php if(is_array($parents)): foreach($parents as $key=>$v): ?><option value="<?php echo ($v['id']); ?>" <?php echo (getSelect(select,$v['id'],$sysitem["bid"])); ?>><?php echo ($v["title"]); ?></option><?php endforeach; endif; ?>
                                        <?php echo ($parentOption); ?>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-10">
                                    <input type="text" class="form-control" name="title" value="<?php echo ($sysitem["title"]); ?>"
                                        required="">
                                </div>
                            </div>
                            <div class="hr-line-dashed"></div>
                            <div class="form-group">
                                <label class="col-sm-2 control-label">连接名称：</label>

                                <div class="col-sm-10">
                                    <input type="text" class="form-control" name="url" value="<?php echo ($sysitem["url"]); ?>">
                                </div>
                            </div>
                            <div class="hr-line-dashed"></div>
                            <div class="form-group">
                                <label class="col-sm-2 control-label">菜单项：</label>
                                <div class="col-sm-10">
                                    <label><input type="radio" name="ismenu" value="0" <?php echo (getSelect(radio,$sysitem["ismenu"],0)); ?>><i></i>否</label>
                                    <label><input type="radio" name="ismenu" value="1" <?php echo (getSelect(radio,$sysitem["ismenu"],1)); ?>><i></i>是</label>
                                </div>
                            </div>
                            <div class="hr-line-dashed"></div>
                            <div class="form-group">
                                ...
                                <div class="col-sm-10">
                                    <input type="text" class="form-control" name="orderby" value="<?php echo ((isset($sysitem['orderby']) && ($sysitem['orderby'] !== ""))?($sysitem['orderby']):getMax('sysitem','orderby')); ?>"
                                        required="">
                                </div>
                            </div>
                            <div class="hr-line-dashed"></div>
                            <div class="form-group">
                                <div class="col-sm-4 col-sm-offset-2">
                                    ...
                                    <button class="btn btn-white" type="button" onclick="location.href='<?php echo U('Sysuser/listsysitem');?>'">返回</button>
                                </div>
                            </div>
                        </form>
                    </div>
