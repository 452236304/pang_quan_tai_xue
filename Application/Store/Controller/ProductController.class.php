...
class ProductController extends BaseController {
    ...
    public function listad(){
		...
		$map['p.company_id'] = $_SESSION['businessID'];
		if($param['top']!==''){
			$map['top']=$param['top'];
		}
		if($param['recommend']!==''){
    ...
    public function modifyad(){
        ...
        if($doinfo == "modify"){
            ...
            // dump($d["typeid"]);die();
			$d['business_cate'] = I("post.business_cate", 0);
			if($d['business_cate']>0){
				$business_cate = D('business_category')->field('pid')->where(array('id'=>$d['business_cate']))->find();
				$d['business_pcate'] = $business_cate['pid'];
			}
			$d['company_id'] = $_SESSION['businessID'];
            ...
            $d["guess_like"] = I("post.guess_like",0);
			if(empty($d["thumb"])){
				$this->error('请上传图片');
			}
			if(!is_http($d['thumb'])){
		...
		//店铺分类
		$map = array('business_id'=>$_SESSION['businessID'],'pid'=>array('gt',0),'status'=>1);
		$business_category = D('business_category')->where($map)->select();
