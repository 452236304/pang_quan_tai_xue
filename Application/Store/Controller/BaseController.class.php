...
class BaseController extends Controller {
	...
	public function checkpower($power){
		...
		if(array_search($power,$sessionpower) && empty($_SESSION["storeID"])){
			return false;
		}else if(!$si){
			return true;
		}else{
			if(array_search($power,$expowerlist)){
				return true;
			}else if(empty($_SESSION["storeID"])){
				return false;
			}else{
				$sysuser = D("SysuserStore");
				$userinfo = $sysuser->getOneById($_SESSION["storeID"]);
				if($userinfo["types"]==0){
					return true;
				}else{
					$classinfo=D('sysuserclass_store')->find($userinfo['typeid']);
