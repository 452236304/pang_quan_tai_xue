...
require_once "Application/Payment/Alipay/AopSdk.php";
...
class AlipayTradeService {
	...
	function pagePay($builder,$return_url,$notify_url) {
		...
		// 首先调用支付api
		$response = $this->aopclientRequestExecute ($request, 1);
	...
	*/
	function transfer($builder) {
		...
		// 首先调用支付api
		$response = $this->aopclientRequestExecute ($request, 3);
	...
	function appPay($builder,$notify_url) {
		...
		// 首先调用支付api
		$response = $this->aopclientRequestExecute ($request, 2);
		// $response = $response->alipay_trade_wap_pay_response;
		return $response;
	}
	 ...
	 * @return $response 支付宝返回的信息
 	*/
	function aopclientRequestExecute($request,$page = 0) {
		...
		{
			$result = $aop->sdkExecute($request,"post");
		}
		else 
		{
	...
	function writeLog($text) {
		...
		$filename = $_SERVER['DOCUMENT_ROOT']."/logs/alipay/".date("Y-m-d").".log";
